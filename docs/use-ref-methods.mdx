...
| `execute()`            | If `options.execution` is set to `'execute'`, this method runs the challenge and shows the widget: before calling it the widget container occupies no space, and after calling it the container gets its visible size back, according to `options.size` and `options.appearance` (e.g. with `appearance: 'interaction-only'` it only grows if an interactive challenge is presented). If the widget is already shown (rendered and executed), this method will not re-render the widget. If the widget got removed (`.remove()`), you need to call `.render()` and then `.execute()`. If `options.execution` is set to `'render'` (default), this method has no effect. | `void`            |
